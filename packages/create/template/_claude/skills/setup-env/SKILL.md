...
### Step 3: Discover existing SES configuration sets
...
Note: Sender configuration (from email, display name, reply-to) is defined per-sequence in each `sequence.config.ts`, not in `.env`.
...
```bash
source .env && echo "Account: $ACCOUNT | Region: $REGION | Stack: $STACK_NAME"
```
...
Note: Sender configuration (from email, display name, reply-to) is defined
per-sequence in each sequence.config.ts, not in .env.
